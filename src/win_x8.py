...
class BurpExtender(IBurpExtender, ITab, IContextMenuFactory):
    ...
    def x8ThreadControl(self,event):
         ...
         while req < len(self.context.getSelectedMessages()):
          ...
          url = '"'+str(self.helpers.analyzeRequest(self.context.getSelectedMessages()[req]).getUrl()).replace('^','^^').replace('\\','^\\').replace('^\\\"','\\\\"').replace('"','\\"').replace('>','^>').replace('<','^<').replace('|','^|').replace('&','^&')+'"'
          ...
          if self.SELECT_MENU == 1:
             if b !='':
              if iRequestInfo.getContentType()!=4:
                 argss = ["x8.exe","--disable-progress-bar","--as-body","-b",b,"-X",X,"-u",str(url),"-v","1","-c",self.threadCsmall,"-w","small-wordlist.txt","--http2","--replay-proxy","http://127.0.0.1:8080"]
              else:
                 argss = ["x8.exe","--disable-progress-bar","-t","json","--as-body","-b",b,"-X",X,"-u",str(url),"-v","1","-c",self.threadCsmall,"-w","small-wordlist.txt","--http2","--replay-proxy","http://127.0.0.1:8080"]
             else:
                 argss = ["x8.exe","--disable-progress-bar","-X",X,"-u",str(url),"-v","1","-c",self.threadCsmall,"-w","small-wordlist.txt","--http2","--replay-proxy","http://127.0.0.1:8080"]
                 
          if self.SELECT_MENU == 2:    
             if b !='':
              if iRequestInfo.getContentType()!=4:
                 argss = ["x8.exe","--disable-progress-bar","--as-body","-b",b,"-X",X,"-u",str(url),"-v","1","-c",self.threadClarge,"-w","large-wordlist.txt","--http2","--replay-proxy","http://127.0.0.1:8080"]
              else:
                 argss = ["x8.exe","--disable-progress-bar","-t","json","--as-body","-b",b,"-X",X,"-u",str(url),"-v","1","-c",self.threadClarge,"-w","large-wordlist.txt","--http2","--replay-proxy","http://127.0.0.1:8080"]
             else:
                 argss = ["x8.exe","--disable-progress-bar","-X",X,"-u",str(url),"-v","1","-c",self.threadClarge,"-w","large-wordlist.txt","--http2","--replay-proxy","http://127.0.0.1:8080"]
                
          if self.SELECT_MENU == 3:    
             if b !='':
              if iRequestInfo.getContentType()!=4:
                 argss = ["x8.exe","--disable-progress-bar","--as-body","-b",b,"-X",X,"-u",str(url),"-v","1","-c",self.threadCsmall,"-w","small-wordlist.txt","--http2","-x",self.proxy,"--replay-proxy","http://127.0.0.1:8080"]
              else:
                 argss = ["x8.exe","--disable-progress-bar","-t","json","--as-body","-b",b,"-X",X,"-u",str(url),"-v","1","-c",self.threadCsmall,"-w","small-wordlist.txt","--http2","-x",self.proxy,"--replay-proxy","http://127.0.0.1:8080"]
             else:
                 argss = ["x8.exe","--disable-progress-bar","-X",X,"-u",str(url),"-v","1","-c",self.threadCsmall,"-w","small-wordlist.txt","--http2","-x",self.proxy,"--replay-proxy","http://127.0.0.1:8080"]
          if self.SELECT_MENU == 4:    
             if b !='':
              if iRequestInfo.getContentType()!=4:
                 argss = ["x8.exe","--disable-progress-bar","--as-body","-b",b,"-X",X,"-u",str(url),"-v","1","-c","3","--custom-parameters","debug" , "_debug" ,"source" , "admin" , "show" , "bot" , "antibot" , "antirobot" , "staging" , "test" , "testing" , "pre" , "pre-staging" , "daily" , "env" , "uat" , "anticrawl" , "recaptcha" ,"captcha", "signing" , "signature" , "enc" , "encryption" , "automation" , "disabled" , "waf" , "disable" , "security" , "dosinglesignon" , "singlesignon" , "dosso" , "sso","--http2","--replay-proxy","http://127.0.0.1:8080"]
              else:
                 argss = ["x8.exe","--disable-progress-bar","-t","json","--as-body","-b",b,"-X",X,"-u",str(url),"-v","1","-c","3","--custom-parameters","debug" ,"_debug" , "source" , "admin" , "show" , "bot" , "antibot" , "antirobot" , "staging" , "test" , "testing" , "pre" , "pre-staging" , "daily" , "env" , "uat" , "anticrawl" , "captcha" , "recaptcha","signing" , "signature" , "enc" , "encryption" , "automation" , "disabled" , "waf" , "disable" , "security" , "dosinglesignon" , "singlesignon" , "dosso" , "sso","--http2","--replay-proxy","http://127.0.0.1:8080"]
             else:
                 argss = ["x8.exe","--disable-progress-bar","-X",X,"-u",str(url),"-v","1","-c","3","--custom-parameters","debug" , "_debug" ,"source" , "admin" , "show" , "bot" , "antibot" , "antirobot" , "staging" , "test" , "testing" , "pre" , "pre-staging" , "daily" , "env" , "uat" , "anticrawl" , "captcha" , "recaptcha","signing" , "signature" , "enc" , "encryption" , "automation" , "disabled" , "waf" , "disable" , "security" , "dosinglesignon" , "singlesignon" , "dosso" , "sso","--http2","--replay-proxy","http://127.0.0.1:8080"]
          ...
          for header in iRequestInfo.getHeaders():
              if i>0 and not header.lower().startswith("Content-Length:".lower()) and not header.lower().startswith("Host:".lower()):
               argss.append('"'+header.replace('^','^^').replace('\\','^\\').replace('^\\\"','\\\\"').replace('"','\\"').replace('>','^>').replace('<','^<').replace('|','^|').replace('&','^&')+'"')
              i=i+1
          argsf = ' '.join(argss)
          if '%s' in url:
